...
class SpinData(ABCData):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        SpinData Class __init__ method
        ...
        super().__init__(values, dims, coords, attrs, spinlab_attrs)
        self.version = version
        self.attrs = attrs
        self.exp_attrs = attrs
        self.spinlab_attrs = spinlab_attrs
        ...
        max_print_attrs = kwargs.pop(
            "max_print_attrs", SpinLAB_CONFIG.getint("CORE", "SpinData_max_print_attrs")
        )
        print_values = kwargs.pop(
            "print_values", SpinLAB_CONFIG.getboolean("CORE", "SpinData_print_values")
        )
    ...
    def __repr__(self):
        """
        Representation of SpinData object
        """
    ...
    def __str__(self):
        """
        String representation of SpinData object
        """
    ...
    def spinlab_info(self):
        """
        Print parameters currently in used in spinlab
        """

        print("-----------------")
        print("SpinLab Attributes")
        print("-----------------")
        if self.spinlab_attrs == {}:
            print("None")
        else:
            longest_key = max(self.spinlab_attrs, key=len)
            maximum_length_of_spinlab_attrs = len(longest_key)
            for x in self.spinlab_attrs:
                spaces = " " * (1 + maximum_length_of_spinlab_attrs - len(x))
                print(
                    "| "
                    + x
                    + spaces
                    + "| "
                    + str(self.spinlab_attrs[x])
                    .replace("{", "")
                    .replace("}", "")
                    .replace("'", "")
                )
    ...
    def show_attrs(
        self, show_exp_info=False, show_spinlab_info=True, show_proc_info=True
    ):
        """
