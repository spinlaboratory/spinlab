...
def create_complex(data, real, imag=None, real_index=0, imag_index=1):
    ...
    With the first dimension ([...,0]) being the real data and the second ([...,1]) the imaginary data. Using the function create_complex the sldata object is converted into a complex data set.
        ...
        .. code-block:: python
            ...
            data = sl.load("MyFile.exp")       # Load example data
    ...
    **kwargs
    ...
    Examples:
        ...
        If you want to select a region for the noise and the signal:
            ...
            >>> snr = sl.signal_to_noise(data,[(-1.23,300.4)],noise_region=[(-400,-240.5),(123.4,213.5)])

        With background subtracted:

            >>> snr = sl.signal_to_noise(data,[(-1.23,300.4)],noise_region=[(-400,-240.5),(123.4,213.5)],remove_background=[(123.4,213.5)])
